...
if not os.path.exists('members.xlsx'):
    pd.DataFrame(columns=['Name', 'Surname', 'ID Number', 'Phone Number', 'Program of Study', 'Department', 'Membership', 'Membership Start Date', 'Membership End Date', 'Member ID']).to_excel('members.xlsx', index=False, engine='openpyxl')

# Function to generate a unique member ID
def generate_member_id():
    if os.path.exists('members.xlsx'):
        existing_data = pd.read_excel('members.xlsx', engine='openpyxl')
...
def add_member():
    if request.method == 'POST':
        ...
        if os.path.exists('members.xlsx'):
            ...
            updated_data.to_excel('members.xlsx', index=False, engine='openpyxl')
        else:
            member_data.to_excel('members.xlsx', index=False, engine='openpyxl')
...
def view_members():
    if os.path.exists('members.xlsx'):
        members = pd.read_excel('members.xlsx', engine='openpyxl')
